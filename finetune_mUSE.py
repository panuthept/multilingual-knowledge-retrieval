        ...
        if self.hard_negative_path is not None:
            with open(self.hard_negative_path, "r") as f:
                for line in f:
                    ...
                    self.hard_negative_ids[question] = hard_negative_ids[:min(int(self.negative_size * self.hard_negative_factor), len(hard_negative_ids)) + 1]
...
def DPRTrainerTF(model, batch_size: int = 32, negative_size: int = 10):
    ...
    flatten_documents = tf.reshape(documents, [-1]) # (batch_size x (negative_size + 1))

    encoded_queries = model(queries)                # (batch_size, )
    encoded_documents = model(flatten_documents)    # (batch_size x (negative_size + 1))

    # NOTE: We don't use in-batch negative because there are many questions that have the same positive document.
    encoded_queries = tf.reshape(encoded_queries, [batch_size, 1, -1])                      # (batch_size, 1, 512)
    encoded_documents = tf.reshape(encoded_documents, [batch_size, negative_size + 1, -1])  # (batch_size, negative_size + 1, 512)

    similarity_scores = tf.matmul(encoded_queries, encoded_documents, transpose_b=True)     # (batch_size, 1, negative_size + 1)
    similarity_scores = tf.reshape(similarity_scores, [batch_size, negative_size + 1])      # (batch_size, negative_size + 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_negative_size", type=int, default=10)
    parser.add_argument("--val_negative_size", type=int, default=999)
    ...
    training_dataset = DPRDataset(
        ...
        negative_size=args.train_negative_size,
    ...
    if os.path.exists(os.path.join(args.dataset_path, "qrel_val.jsonl")):
        validation_dataset = DPRDataset(
            ...
            batch_size=1,
            shuffle=False,
            negative_size=args.val_negative_size,
            hard_negative_path=os.path.join(args.dataset_path, "bm25_top1000.jsonl"),
    ...
    training_model = DPRTrainerTF(muse, batch_size=args.batch_size, negative_size=args.train_negative_size)
    training_model.summary()
    training_model.compile(optimizer=AdamW(learning_rate=args.learning_rate), loss=tf.keras.losses.CategoricalCrossentropy(from_logits=True, axis=-1), metrics=cal_mrr)
    print("Validation model:")
    validation_model = DPRTrainerTF(muse, batch_size=1, negative_size=args.val_negative_size)
    validation_model.summary()
    validation_model.compile(optimizer=AdamW(learning_rate=args.learning_rate), loss=tf.keras.losses.CategoricalCrossentropy(from_logits=True, axis=-1), metrics=cal_mrr)
    ...
    # Validation step (initial score)
    if "validation_dataset" in locals():
        val_losses = []
        val_metrics = []
        for val_batch_idx in trange(validation_dataset.num_batches, disable=not args.verbose, desc="Validation step"):
            (queries, documents), labels = validation_dataset[val_batch_idx]
            val_outputs = validation_model.test_on_batch(
                x=[queries, documents],
                y=labels,
                return_dict=True,
            )
        ...
        if args.verbose:
            print(f"Valiation_Loss: {val_losses}, Valiation_MRR: {val_metrics}")
    else:
        tf.saved_model.save(hub_load, os.path.join(args.save_dir, args.save_name, "best_model"))
    ...
    for epoch_idx in trange(args.epochs, desc="Epoch"):
        for train_batch_idx in trange(training_dataset.num_batches, desc="Training step"):
            ...
            train_outputs = training_model.train_on_batch(
                x=[queries, documents],
                y=labels,
                return_dict=True,
            )
            ...
            with summary_writer.as_default():
                ...
                tf.summary.scalar("Learning_Rate", K.get_value(training_model.optimizer.learning_rate), step)
        ...
        K.set_value(training_model.optimizer.learning_rate, K.get_value(training_model.optimizer.learning_rate) * (1 - args.learning_rate_decay))
